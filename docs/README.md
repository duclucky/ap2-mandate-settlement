...
- Status: V2_FULL_LIFECYCLE_CAPTURED
...
| Full lifecycle | PASS | v2 deploy, open mandate, accept, open dispute, adjudication, withdrawal, and canonical reads are captured on Studionet. |
...
| Evidence facts must be issuer-signed | `adjudicate_dispute`, Ed25519 verification | `get_dispute` | `test_fabricated_hash_valid_unsigned_evidence_is_unverifiable`, `test_forged_signature_is_unverifiable` | Studionet v2 lifecycle evidence |
...
- v2 lifecycle status: deploy, `open_mandate`, `accept_mandate`, `open_dispute`, `adjudicate_dispute`, and `withdraw_credit` finalized. Canonical reads show `VIOLATION`, `REFUNDED`, zero locked escrow, zero locked dispute bonds, and zero withdrawable credits after withdrawal.
...
- [x] Adversarial tests.
- [x] Real network lifecycle final adjudication and withdrawal.
- [x] Canonical settlement evidence.
