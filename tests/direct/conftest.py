...
def _install_windows_stdin_patch() -> None:
    ...
    if getattr(loader, "_ap2_settlement_windows_stdin_patch", False):
        return
    ...
    def inject_message_to_fd0(vm: VMContext) -> None:
        ...
        try:
            ...
            os.dup2(fd, 0)
            vm._ap2_settlement_stdin_temp_path = path
        finally:
            os.close(fd)
    ...
    def cleanup_after_deactivate(self: VMContext) -> None:
        try:
            ...
            path = getattr(self, "_ap2_settlement_stdin_temp_path", None)
            if path:
                try:
                    os.unlink(path)
                except FileNotFoundError:
                    pass
                self._ap2_settlement_stdin_temp_path = None

    loader._inject_message_to_fd0 = inject_message_to_fd0
    loader._ap2_settlement_windows_stdin_patch = True
